...
import { Button, EmptyState, PageShell, SkeletonTable, Toast, GlassPanel, Modal } from '../components/ui';
          ...
          <section className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            {[
              ...
              <GlassPanel
                ...
                variant="sm"
                padding="sm"
                motionProps={{ initial: { opacity: 0, y: 10 }, animate: { opacity: 1, y: 0 }, transition: { duration: 0.3, delay: index * 0.05 } }}
              >
                    ...
                    return (
                      <button
                        ...
                        className={`whitespace-nowrap rounded-full px-4 py-2 text-sm font-semibold transition duration-sm focus:outline-none focus:ring-4 focus:ring-primary-500/10 ${
                          isActive ? 'bg-slate-950 text-white shadow-elev-1' : 'border border-slate-200 bg-white text-slate-600 hover:bg-slate-50'
                        }`}
                      >
                        {category}
                      </button>
                      ...
                      <AnimatePresence initial={false}>
                        {filteredCourses.map((course, index) => (
                          <MotionTr
                            ...
                            className="text-sm text-slate-700 transition duration-sm hover:bg-slate-50"
                          >
                            ...
                            <td className="px-5 py-4">
                              <span className={`status-badge ${statusTone[course.status]}`}>
                                {course.status}
                              </span>
                            </td>
                            ...
                            <td className="px-5 py-4">
                              <div className="flex items-center justify-end gap-2">
                                <button type="button" className="rounded-full border border-slate-200 bg-white px-3 py-2 text-xs font-semibold text-slate-700 transition hover:bg-slate-50 hover:shadow-sm" onClick={() => openEdit(course)}>
                                  Edit
                                </button>
        ...
        <div className="space-y-5">
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            {stepList.map((step, index) => {
              ...
              return (
                <div key={step} className={`rounded-2xl border px-4 py-4 text-center text-sm font-semibold transition ${isActive ? 'border-primary-500 bg-primary-500 text-white' : 'border-slate-200 bg-slate-50 text-slate-600'}`}>
                  {step}
                </div>
              );
            })}
